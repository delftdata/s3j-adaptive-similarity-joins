...
do
  ...
  python ~/ssj-experiment-results/get_flink_metrics_cj.py -en "$name" -om $metrics
  printf 'Experiment finished... \n'

  printf '\nCalculating stats...\n'
  curl http://coordinator:30080/start_stats_clusterjoin?parallelism=1
  sleep 20
  python ~/ssj-experiment-results/monitor_cj_stats.py
  printf '\nStats calculated\n'
