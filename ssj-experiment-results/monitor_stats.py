...
if __name__ == "__main__":
    ...
    stats_jobid_request = requests.get(f'http://coordinator:30080/get_stats_jobid')
    stats_id = stats_jobid_request.text

    r = requests.get(f'http://flink-rest:30080/jobs/{stats_id}/plan')
    ...
    while 1:
        ...
        for vertex in vertices:
            b = requests.get(f'http://flink-rest:30080/jobs/{stats_id}/vertices/{vertex}/subtasks/metrics'
                             f'?get={metrics}')
            # print("sink requests\n")
        ...
        for s in source.keys():
            b = requests.get(f'http://flink-rest:30080/jobs/{stats_id}/vertices/{s}/subtasks/metrics'
                             f'?get={source[s]}')
